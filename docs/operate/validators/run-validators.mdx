...
<Info>
  ...
  Terraform](/docs/operate/deploy-with-terraform) or a community-submitted
  [Ansible playbook](https://github.com/polkachu/hyperlane) instead. This
  process will automatically create agent keys, Validator buckets, permissions
  and any other auxiliary setup required to run a Validator cluster on AWS.
</Info>
...
- **Configure multiple RPCs:** Use [`customRpcUrls`](/docs/operate/config-reference#chainschain_namecustomrpcurls).
- **Configure gRPC endpoints:** Use [`customGrpcUrls`](/docs/operate/config-reference#chainschain_namecustomgrpcurls).
