...
- **Epoch Length**: Rewards are calculated algorithmically and automatically distributed per epoch, which is currently set to \~1 month. This duration reflects the maximum settlement delay on default [Interchain Security Modules (ISMs)](/learn/protocol/ISM/modular-security) validator set chains.
...
Once mutual interest is established via a shared communication channel, Validators can proceed with deployment (they may also deploy earlier if they choose). See [Run Validators](/operate/validators/run-validators).
