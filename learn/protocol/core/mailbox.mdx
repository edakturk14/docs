...
- To [**send**](/learn/reference/messaging/send) interchain messages, call the `dispatch` function.
- To [**receive**](/learn/reference/messaging/receive) interchain messages, implement the `handle` function.
...
See the [`Message` library](/learn/reference/libraries/message) for more information on the message encoding.
...
To deliver interchain messages, the [relayer](/learn/protocol/agents/relayer) calls `Mailbox.process()`.
