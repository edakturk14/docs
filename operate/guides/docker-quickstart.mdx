...
Follow the guide [here](/operate/key-management/set-up-agent-keys#2-aws-kms) for creating agent keys with AWS KMS.

## 2. Create S3 bucket for signatures

Follow the guide [here](/operate/validators/validator-signatures-aws) for creating and configuring an S3 bucket for your validator to write signatures to.
...
If you are an AVS operator, follow the guide [here](/operate/guides/avs-operator-guide#4-register-your-operator) to register with the Hyperlane AVS.
...
You can read more about Agent Configs [here](/operate/config/agent-config).
