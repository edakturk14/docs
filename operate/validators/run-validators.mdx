...
<Info>
  ...
  [Overview section](/operate/overview-agents).
</Info>
...
- **Secure Signing Key**
  ...
  - The Hyperlane Validator agent currently supports signing with AWS KMS keys that are accessed via API keys/secrets as well as hexadecimal plaintext keys. See more under [agent keys](/operate/key-management/set-up-agent-keys).
...
- **RPC Node**
  ...
  - Validators make simple view calls to read merkle roots from the [Mailbox](/docs/protocol/mailbox) contract on the chain they are validating for. You must use your own RPC urls not any public ones.
...
1. Created a key for your Validator to sign with, see the [Agent Keys](/operate/key-management/set-up-agent-keys) documentation.
2. Set up the destination for your Validator signatures to be posted, see the [AWS Signatures Bucket Setup](/operate/validators/validator-signatures-aws) guide.

### Configuration[​](#configuration "Direct link to Configuration")

<Info>
  Experienced operators may prefer to [deploy agents with
  Terraform](/operate/deploy-with-terraform) or a community-submitted [Ansible
  playbook](https://github.com/polkachu/hyperlane) instead. This process will
  automatically create agent keys, Validator buckets, permissions and any other
  auxiliary setup required to run a Validator cluster on AWS.
</Info>

#### RPC Configuration[​](#rpc-configuration "Direct link to RPC Configuration")
...
- **Configure multiple RPCs:** Use [`customRpcUrls`](/operate/config-reference#chainschain_namecustomrpcurls).

- **RPC Selection Mode ([`rpcConsensusType`](/operate/config-reference#chainschain_namerpcconsensustype))**:
...
- **Configure multiple RPCs:** Use [`customRpcUrls`](/operate/config-reference#chainschain_namecustomrpcurls).
- **Configure gRPC endpoints:** Use [`customGrpcUrls`](/operate/config-reference#chainschain_namecustomgrpcurls).
...
- **Configure RPCs:** Use [`customRpcUrls`](/operate/config-reference#chainschain_namecustomrpcurls).
...
<Info>
  ...
  configuration. Take a look at the [agent configuration](/operate/agent-config)
  page and the [configuration reference](/operate/config-reference) for a full
  list of configuration possibilities.
</Info>

You can also provide the path to additional configuration files as a comma separated list with the [`CONFIG_FILES` environment variable](/operate/config-reference#config_files). If you choose to run in Docker, see the docker section of [agent configuration](/operate/agent-config) for tips on mounting your config files into your Docker container.

### Setup-specific configuration[​](#setup-specific-configuration "Direct link to Setup-specific configuration")
...
## Start Validating[​](#start-validating "Direct link to Start Validating")
...
### Announcing your Validator[​](#announcing-your-validator "Direct link to Announcing your Validator")

The [Relayer](/operate/relayer/run-relayer) needs to know where to find your Validator's signatures. Your Validator will automatically attempt to announce itself by writing to the `ValidatorAnnounce` contract on the chain that you're validating.
...
### Additional Steps for Multiple Validators[​](#additional-steps-for-multiple-validators "Direct link to Additional Steps for Multiple Validators")

#### 1. Create an Agent Configuration[​](#1-create-an-agent-configuration "Direct link to 1. Create an Agent Configuration")
...
#### 2. Run Each Validator[​](#2-run-each-validator "Direct link to 2. Run Each Validator")
...
## Design reference[​](#design-reference "Direct link to Design reference")

[Edit this page](https://github.com/hyperlane-xyz/v3-docs/tree/main/docs/operate/validators/run-validators.mdx)
