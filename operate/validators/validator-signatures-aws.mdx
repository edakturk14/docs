...
<Info>
  ...
  guide](/guides/chains/deploy-hyperlane-with-local-agents) instead.
</Info>

### 1. Create an AWS IAM user, and a signing key

Follow the instructions in [Agent Keys](/operate/key-management/set-up-agent-keys) to generate an AWS IAM user and either a KMS or a hexadecimal key. You will use this key's credentials in the following steps.
...
<Info>
  ...
  module](/cs/operate/deploy-with-terraform#iam--kms) instead to create the S3
