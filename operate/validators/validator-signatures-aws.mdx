...
<Info>
  These instructions are for a production environment where Validator keys exist
  in AWS's Key Management Service and Validator signatures are posted publicly
  in an S3 bucket. If you're only intending to run a Validator for testing or
  development purposes, consider following the [local setup
  guide](/docs/guides/deploy-hyperlane-local-agents) instead.
</Info>
...
<Info>
  ...
  module](/docs/operate/deploy-with-terraform#iam--kms) instead to create the S3
  bucket with the correct permissions.
</Info>
