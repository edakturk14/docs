---
title: "Overview"
description: "The Hyperlane protocol decouples the transport layer from the security layer of cross-chain message passing. To run a deployment, it relies on offchain agents that observe onchain activity and carry out either the transport or security aspects of the protocol."
---

1. Hyperlane [Validators](#validators) are light offchain agents responsible for security - they observe messages on an origin chain's [Mailbox](/docs/protocol/mailbox) and if needed sign a merkle root that attests the current state of the mailbox.
...
👉 Learn more about Validators [here](/docs/protocol/agents/validators).
...
Hyperlane Relayers are configured to relay messages between one or more origin chains and destination chains. Every Hyperlane message involves two transactions:
...
👉 Learn more about Relayers [here](/docs/protocol/agents/relayer).

[Edit this page](https://github.com/hyperlane-xyz/v3-docs/tree/main/docs/operate/overview-agents.mdx)
