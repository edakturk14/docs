...
1. Hyperlane [Validators](#validators) are light offchain agents responsible for security - they observe messages on an origin chain's [Mailbox](/learn/protocol/core/mailbox) and if needed sign a merkle root that attests the current state of the mailbox.
2. [Relayers](#relayer) fulfil the message transport requirement of the protocol. They aggregate off-chain security metadata for the [`IInterchainSecurityModule` interface](/learn/protocol/ISM/modular-security) and deliver messages to their recipients.

## Validators

[Validators](/learn/protocol/agents/validators) fulfil the security requirement of the protocol, as part of the [Multisig ISM](/learn/protocol/ISM/standard-ISMs/multisig-ISM) or the [Hyperlane AVS](/learn/protocol/ISM/economic-security/hyperlane-avs), by attesting to the validity of [Mailbox](/learn/protocol/core/mailbox) messages and making their signatures available to a Relayer.
...
<Tip>Learn more about Validators [here](/learn/protocol/agents/validators)</Tip>

## Relayer

A Hyperlane [Relayer](/learn/protocol/agents/relayer) delivers interchain messages to their recipients.
...
1. To [`dispatch`](/learn/reference/messaging/send) and send a message on the origin chain.
2. To [`process`](/learn/reference/messaging/receive) and deliver the message on destination chain.
...
- It checks the [Interchain Security Module (ISM)](/learn/protocol/ISM/modular-security) specified by the recipient to gather the necessary metadata to secure the message. For example, it may retrieve Multisig ISM signatures from Validator checkpoint storage.
...
<Tip>Learn more about Relayers [here](/learn/protocol/agents/relayer).</Tip>
